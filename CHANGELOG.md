...
### Added
- **`ministack` CLI now accepts `-H/--host` and `-p/--port`** — the bind interface and port were previously only configurable via env vars (`GATEWAY_PORT` / `EDGE_PORT`), which were not discoverable from `ministack --help`. The new flags surface in the help output, follow standard `--host` / `--port` UX, and take precedence over the env vars when set. Defaults are unchanged (`0.0.0.0:4566`); the env vars continue to work as a fallback so existing setups keep behaving identically. The help text also calls out the distinction between `--host` (the bind interface, e.g. `127.0.0.1` to restrict to loopback) and the existing `MINISTACK_HOST` env var (the virtual hostname used for S3 virtual-host / execute-api URL matching) so the two don't get confused. The "port already in use" error message now also points at `ministack --port 4567` instead of the old `GATEWAY_PORT=4567 ministack` form.
