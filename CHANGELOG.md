...
- **Lambda Docker images upgraded to AWS RIE** — Docker executor now uses official AWS Lambda Runtime Interface Emulator images (`public.ecr.aws/lambda/*`) instead of generic `python:*-slim`/`node:*-slim` images. Added `nodejs22.x`, `nodejs24.x`, `python3.14` runtimes. Contributed by @fzonneveld (#302)
