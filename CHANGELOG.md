...
### Added
- **API Gateway v2 WebSocket APIs** — fully-live WebSocket support on the execute-api host (`ws://{apiId}.execute-api.{host}:{port}/{stage}`). Implements the four pieces needed to run a WebSocket-driven app locally:
  - **Control plane**: `CreateApi(ProtocolType=WEBSOCKET)` with the correct `RouteSelectionExpression` default (`$request.body.action`); full `RouteResponse` and `IntegrationResponse` CRUD (`/v2/apis/{apiId}/routes/{routeId}/routeresponses` and `/v2/apis/{apiId}/integrations/{integrationId}/integrationresponses`).
  - **Handshake + routing**: ASGI `websocket` scope handled end-to-end. On connect, the `$connect` route Lambda runs (with the handshake's `queryStringParameters` / `multiValueQueryStringParameters` exposed on the event, so token-gated connect hooks work like AWS); 2xx accepts the upgrade, non-2xx rejects it. Incoming frames are dispatched via the API's `RouteSelectionExpression` (default `$request.body.action`) with `$default` as the catch-all. Lambda event shape matches AWS WebSocket v2 proxy: `requestContext.connectionId` / `routeKey` / `eventType` (`CONNECT`/`MESSAGE`/`DISCONNECT`) / `messageId` / `identity.sourceIp`. Lambda response `body` is forwarded back on the same socket. `$disconnect` is fired fire-and-forget on close.
  - **Integration types**: `AWS` / `AWS_PROXY` dispatch to Lambda via the warm worker pool. `MOCK` is also supported — the first matching `IntegrationResponse.responseTemplates` entry (preferring `$default`) is returned on the socket without running any Lambda. Unsupported types (e.g. `HTTP_PROXY`, which AWS itself does not allow on WebSocket APIs) log a warning and return nothing.
  - **`@connections` management API**: `POST /{stage}/@connections/{connectionId}` (PostToConnection), `GET` (GetConnection), `DELETE` (DeleteConnection). Served on the execute-api host. Unknown or closed connections return `410 GoneException`. `PostToConnection` pushes into a per-connection outbox that the WS session drains, so server-side push works across multiple clients concurrently.
  - **Multi-tenancy**: connections carry their owning account id; @connections lookups reject cross-API requests. The WS handler runs inside the owning account's context so Lambda invocations scope their resources correctly.
  - Closes [#383](https://github.com/ministackorg/ministack/issues/383). No new dependencies — WebSocket upgrade is served by the existing hypercorn ASGI server and tested with a stdlib-only client.
